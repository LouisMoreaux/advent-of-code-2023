with
   function calculate_points(p_count in number)
      return number
   is
      l_return number := 0;
   begin
   ...
   raw_data as (
      select rn as id, regexp_replace(val, '^Card[ ]{1,}[0-9]{1,}:') as column_value
        from json_table('["'
   ...
   extracted_data as (
      select sd.id, sd.rn, jt.rn rn2, jt.val, regexp_replace(trim(sd.val), '[^0-9]', ',') as t
        from splitted_data sd,
             json_table('['
                || replace(regexp_replace(trim(sd.val), '[^0-9]', ','), ',,', ', null,')
                || ']', '$[*]'
                columns (
                   rn for ordinality,
                   val varchar2 path '$'
...
select sum(calculate_points(cnt))
  from winner_count