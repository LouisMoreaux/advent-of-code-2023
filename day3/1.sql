with
   ...
   is
      ...
      l_special_char_count    integer;
      l_special_char_pos      integer;

      l_found_in_previous_row boolean;
   begin
      l_count_number := regexp_count(p_column_value, '([0-9]{1,})');
      for i in 1..l_count_number
      loop
         l_str_num               := regexp_substr(p_column_value, '([0-9]{1,})', 1, i);
         l_str_pos               := regexp_instr(p_column_value, '([0-9]{1,})', 1, i);
         l_str_length            := length(l_str_num);

         l_start_pos             := 1;
         l_end_pos               := length(p_column_value);

         if l_str_pos > 1 then
            l_start_pos := l_str_pos - 1;
         end if;

         if l_str_pos + l_str_length < length(p_column_value) then
            l_end_pos := l_str_pos + l_str_length;
         end if;
      
         -- Look if there is special character in start and end pos same row
         if (
               regexp_count(substr(p_column_value, l_start_pos, 1), '[^0-9.]') > 0
            )
         then
            l_row_sum := l_row_sum + to_number(l_str_num);
            continue;
         end if;

         if (
               regexp_count(substr(p_column_value, l_end_pos, 1), '[^0-9.]') > 0
            )
         then
            l_row_sum := l_row_sum + to_number(l_str_num);
            continue;
         ...
         -- Look in the preceding row
         l_found_in_previous_row := false;
         if p_previous_column_value is not null then
            l_special_char_count := regexp_count(p_previous_column_value, '[^0-9.]');

            for j in 1..l_special_char_count
            loop
               l_special_char_pos := regexp_instr(p_previous_column_value, '[^0-9.]', 1, j);
               if (l_special_char_pos >= l_start_pos and l_special_char_pos <= l_end_pos) then
                  l_row_sum               := l_row_sum + to_number(l_str_num);
                  l_found_in_previous_row := true;
                  exit;
               end if;
            end loop;
         end if;
         ...
         -- Look at next row
         if (l_found_in_previous_row = false) then
            if p_next_column_value is not null then
               l_special_char_count := regexp_count(p_next_column_value, '[^0-9.]');

               for j in 1..l_special_char_count
               loop
                  l_special_char_pos := regexp_instr(p_next_column_value, '[^0-9.]', 1, j);
                  if (l_special_char_pos >= l_start_pos and l_special_char_pos <= l_end_pos) then
                     l_row_sum := l_row_sum + to_number(l_str_num);
                     exit;
                  end if;
